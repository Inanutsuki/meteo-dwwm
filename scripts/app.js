...
function prepareWeatherData(weatherData) {
    let respCondition = weatherData.current_condition;
    ...
    for (let i = 0; i < 5; ++i) {
        const dayData = {
            ...
            currentConditionIconDay: weatherData["fcst_day_" + i].icon,
    ...
    for (let i = 0; i < 5; ++i) {
        let rspData = weatherData["fcst_day_" + i].hourly_data;
...
function resetHourlyDataAndHTML() {
 ...
 * 
 * @param {PositionError} err L'erreur générée
 */
 ...
 * successgetGPSPosition() se lance, si non la fonction errorgetGPSPosition() se lance.
 */
...
/**
 * Fonction permettant d'ajouter la classe "active" a l'onglet et au bloc actif et de
...
//     if (value.length > 2) {
//         for (i = 0, arrNew = []; i < $cities.length; ++i) {
//             inputVal = $cities[i].$citiesNames.toUpperCase();
//             if (inputVal.startsWith(value)) {
//                 arrNew.push($cities[i]);
//                 console.log(arrNew);
//             }
//         }
//     } else {
...
// inputCity.addEventListener('input', autoCompletion);

/** Fin d'autocomplétion **/